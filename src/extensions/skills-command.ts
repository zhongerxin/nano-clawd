...
function isPathInWorkspace(filePath: string | undefined, cwd: string): boolean {
...
export default function skillsCommandExtension(pi: ExtensionAPI) {
  pi.registerCommand("skills", {
    ...
    handler: async (_args, ctx) => {
      ...
      const workspaceSkills = allSkills.filter(
        (skill) => skill.location === "project" || isPathInWorkspace(skill.path, ctx.cwd),
      )
